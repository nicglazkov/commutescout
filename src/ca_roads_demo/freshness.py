...
So this checks every source on a timer and logs ``feed stale:`` at
ERROR when one has not refreshed within its own limit, once when it
goes stale and again every hour it stays that way, and ``feed
recovered:`` when it comes back. A log-based alert policy matches the
ERROR line.
"""
...
# refreshed in this long has been failing for most of it.
STATE_STALE_S = 45 * 60
...
def report(stale: dict[str, str], now: float | None = None) -> None:
    ...
    for label, why in stale.items():
        ...
        if now - _last_logged.get(label, -1e18) >= REPEAT_S:
            minutes = (now - _stale_since[label]) / 60
            log.error("feed stale: %s: %s (flagged for %.0f min)", label, why, minutes)
